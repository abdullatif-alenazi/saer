...
function riyadhDateParts(date:Date){const parts=new Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Riyadh",year:"numeric",month:"numeric",day:"numeric",hour:"numeric",minute:"numeric",hourCycle:"h23"}).formatToParts(date),value=(type:string)=>Number(parts.find(part=>part.type===type)?.value??0);return{year:value("year"),month:value("month"),day:value("day"),hour:value("hour"),minute:value("minute")}}
function julianDay(date:Date){const parts=riyadhDateParts(date),month=parts.month,day=parts.day;let year=parts.year,adjustedMonth=month;if(month<=2){year-=1;adjustedMonth+=12}const a=Math.floor(year/100),b=2-a+Math.floor(a/4);return Math.floor(365.25*(year+4716))+Math.floor(30.6001*(adjustedMonth+1))+day+b-1524.5}
...
export function minuteOfDay(date:Date){const parts=riyadhDateParts(date);return parts.hour*60+parts.minute}
export function resolveTimeWindow(date:Date):TimeWindow{const times=getRiyadhPrayerTimes(date),starts=[times.fajr,times.dhuhr,times.asr,times.maghrib,times.isha],names=["الظهر","العصر","المغرب","الليل","الفجر"];const minute=minuteOfDay(date);let index=4;if(minute>=starts[0]&&minute<starts[1])index=0;else if(minute>=starts[1]&&minute<starts[2])index=1;else if(minute>=starts[2]&&minute<starts[3])index=2;else if(minute>=starts[3]&&minute<starts[4])index=3;const afterMidnight=index===4&&minute<starts[0],startMinute=index===4&&afterMidnight?starts[4]-1440:starts[index],endMinute=index===4?starts[0]+1440:starts[index+1],normalized=afterMidnight?minute+1440:minute;return{index,startMinute,endMinute,remainingMinutes:Math.max(0,endMinute-normalized),nextName:names[index],times}}
