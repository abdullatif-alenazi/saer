...
type Mode="ready"|"work"|"rest"|"exerciseDone"|"sessionDone";
...
export default function WorkoutRunner({activity,onExit,onFinish}:{activity:PlannedActivity;onExit:()=>void;onFinish:(minutes:number,repetitions:number)=>void}){
 ...
 useEffect(()=>{if(mode!=="work"&&mode!=="rest")return;const timer=window.setInterval(()=>{setSeconds(value=>mode==="work"&&exercise?.measurement==="repetitions"?value+1:Math.max(0,value-1));mode==="work"?setWorkTotal(value=>value+1):setRestTotal(value=>value+1)},1000);return()=>clearInterval(timer)},[mode,exercise?.measurement]);
 useEffect(()=>{if(seconds!==0)return;if(mode==="rest")setMode("ready");if(mode==="work"&&exercise?.measurement==="duration")completeSet()},[seconds,mode]);
 ...
 const startSet=()=>{setSeconds(exercise.measurement==="duration"?(exercise.durationSeconds??30):0);setMode("work")};
 ...
 const status=mode==="ready"?"جاهز":mode==="work"?"الجولة":mode==="rest"?"راحة":"تم";
 return <section className={`workout-runner workout-${mode}`} aria-live="polite">
  {mode!=="sessionDone"?<>
   ...
   {(mode==="work"||mode==="rest")&&<div className="workout-clock" style={{"--timer-progress":`${progress}%`} as React.CSSProperties}><div><strong>{clock(seconds)}</strong><span>{mode==="rest"?`راحة قبل الجولة ${ar(setIndex)}`:exercise.measurement==="duration"?"باقي من الجولة":"وقت الجولة"}</span></div></div>}
   ...
   {mode==="rest"&&<div className="rest-actions"><button onClick={()=>setSeconds(value=>value+30)}>+ ٣٠ ثانية</button><button onClick={()=>{setSeconds(0);setMode("ready")}}>تخطي الراحة</button></div>}
